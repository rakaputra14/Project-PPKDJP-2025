...
if (isset($_POST['save'])) {
    ...
    $insert = mysqli_query($conn, "INSERT INTO levels (level_name)
    VALUES('$level_name')");
    if ($insert) {
        header("location:?page=levels&add=success");
    }
}

$id = isset($_GET['edit']) ? $_GET['edit'] : '';
$queryEdit = mysqli_query($conn, "SELECT * FROM levels WHERE id = '$id'");
...
if (isset($_POST['edit'])) {
    ...
    $update = mysqli_query($conn, "UPDATE levels 
    SET level_name ='$level_name' WHERE id ='$id'");
    if ($update) {
        header("location:?page=levels&update=success");
    }
}
